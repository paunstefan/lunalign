...
impl SerFile {
    pub fn decode(ser_file: &str, output_dir: &Path) {
        let f = File::open(ser_file).unwrap();
        let mut reader = BufReader::new(f);

        let mut header_buffer = [0u8; 178];
        reader.read_exact(&mut header_buffer);

        let color = i32::from_le_bytes(header_buffer[18..22].try_into().unwrap());
        let endianess = i32::from_le_bytes(header_buffer[22..26].try_into().unwrap());
        let width = i32::from_le_bytes(header_buffer[26..30].try_into().unwrap());
        let height = i32::from_le_bytes(header_buffer[30..34].try_into().unwrap());
        let pixel_depth = i32::from_le_bytes(header_buffer[34..38].try_into().unwrap());
        let frame_count = i32::from_le_bytes(header_buffer[38..42].try_into().unwrap());
        // The other fields are not relevant for the moment

        let header = SerHeader {
            name: ser_file.to_string(),
            color,
            endianess,
            width,
            height,
        ...
        let image_type = match header.pixel_depth {
            ...
            _ => {
                panic!("Unsupported pixel depth {}", header.pixel_depth);
            }
        };
        ...
        for i in 0..header.frame_count {
            ...
            reader.read_exact(&mut frame_buffer).unwrap();
            let output_filename = output_dir.join(format!("decoded_{:04}.fits", i));
            ...
            let mut fptr = if output_filename.exists() {
                FitsFile::edit(&output_filename).unwrap()
            } else {
            ...
            match image_type {
                ImageType::UnsignedByte => {
                    ...
                    hdu.write_image(&mut fptr, &frame_buffer).unwrap();
                }
